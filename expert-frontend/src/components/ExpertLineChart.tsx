...
export type ExpertSeriesPoint = {
  ...
  // valores em BRL (já vindo do backend como number)
  depositsBRL?: number;
  revBRL?: number;
  salesBRL?: number;
  trafficBRL?: number;
...
// ✅ calcula min/max de um conjunto de keys dentro do dataset (garante 0 no domínio)
...
// ✅ ticks bonitinhos só >= 0 (pra eixo de contagem)
      ...
      <div className="space-y-1.5">
        {payload
          ...
          .map((p, idx) => {
            ...
            const val = kind === 'money' ? formatBRL(safeNumber(p.value)) : formatInt(safeNumber(p.value));
...
export default function ExpertLineChart({
  ...
  /**
   * ✅ ALINHAMENTO DO ZERO (money x count)
   * - Se o eixo money tiver negativo (REV), o 0 do money sobe.
   * - Para o 0 do eixo count ficar na MESMA altura, criamos um "mínimo negativo invisível"
   *   no domínio do count, mas continuamos exibindo ticks só >= 0.
   */
  const domains = useMemo(() => {
    ...
    // domínio do money (pode ser negativo por causa do REV)
    const moneyDomain: [number, number] = [money.min, money.max];

    // domínio do count: por padrão 0..max
    let countMax = Math.max(0, count.max);
    if (countMax === 0) countMax = 1; // evita range 0
    let countMin = 0;

    // se money tem parte negativa, alinha o 0 criando parte negativa "invisível" no count
    if (hasMoney && hasCount && money.min < 0) {
      const moneyRange = money.max - money.min; // > 0
      const zeroPos = (0 - money.min) / moneyRange; // 0..1 (posição do zero no eixo money)

      // queremos: (0 - countMin) / (countMax - countMin) = zeroPos
      // => countMin = - (zeroPos * countMax) / (1 - zeroPos)
      const eps = 1e-9;
    ...
    // ticks só >= 0 pro eixo count (mesmo com domain negativo)
        ...
        <div className="h-[320px] w-full overflow-hidden">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              ...
              /* ✅ ESSENCIAL: força o SVG (Surface) a NÃO vazar */
            ...
            >
              ...
              {hasMoneyAxis ? (
                <YAxis
                  ...
                  axisLine={{ stroke: 'rgba(255,255,255,0.12)' }}
                  tickLine={{ stroke: 'rgba(255,255,255,0.12)' }}
                  tickFormatter={(v) => formatBRL(safeNumber(v))}
                  width={84}
                  // ✅ domínio fixado pra garantir 0 incluído e permitir negativo (REV)
              ...
              {hasCountAxis ? (
                <YAxis
                  ...
                  axisLine={{ stroke: 'rgba(255,255,255,0.12)' }}
                  tickLine={{ stroke: 'rgba(255,255,255,0.12)' }}
                  tickFormatter={(v) => formatInt(safeNumber(v))}
                  width={54}
                  allowDecimals={false}
                  // ✅ domain com parte negativa "invisível" para alinhar o 0 com o eixo money
              ...
              {/* ✅ IMPORTANTE: remove cursor/linha que o Tooltip desenha */}
              <Tooltip content={<CustomTooltip />} cursor={false} />

              <Legend wrapperStyle={{ display: 'none' }} />

              {/* MONEY (com preenchimento embaixo) */}
              {enabled.revBRL && (
                <Area
                  yAxisId="money"
                  type="monotone"
                  ...
                  dot={{ r: 3 }}
                  activeDot={{ r: 6 }}
                  /* ✅ evita path “voando” no primeiro render */
              ...
              {/* COUNT (com preenchimento embaixo) */}
              {enabled.salesCount && (
                <Area
                  yAxisId="count"
                  type="monotone"
